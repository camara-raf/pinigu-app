    ...
    if consolidated_df.empty:
        ...
        with col1:
            ...
            if not cash_flow_df.empty:
                ...
                last_14 = cash_flow_monthly['YearMonth'].unique()[-14:]
                start_range = last_14[0]
                end_range = last_14[-1]

                fig_cash.update_xaxes(
                    tickangle=55,
                    tickmode='linear',
                    dtick="M1",
        ...
        with col2:
            ...
            if not spending_df.empty:
                ...
                # Determine last 14 months for initial view
                last_14 = spending_by_month['YearMonth'].unique()[-14:]
                start_range = last_14[0]
                end_range = last_14[-1]
        ...
        st.divider()
        with col5:
            st.subheader("📈 Monthly Balance Table")     
            #Exclude None Transactions
            month_balance = consolidated_df.copy()
            ...
            month_balance = month_balance.sort_values(['YearMonth', 'Bank', 'Account'])
            
            month_balance['Rolling Sum'] = month_balance.groupby(['Bank','Account'])['Amount'].cumsum()
            
            st.dataframe(month_balance, use_container_width=True, hide_index=True)
        
        with col6:
            st.subheader("To be defined")
